...
Button::Button(uint32_t periph, GPIO_TypeDef* port, uint32_t pin, uint8_t puEnable, uint8_t invert, uint32_t dbTime)
{
	_port = port;
    _pin = pin;
    _puEnable = puEnable;
    ...
    _state =  GPIO_ReadInputDataBit(_port, _pin);
...
uint8_t Button::read(void)
{
    ...
    ms = millis();
    pinVal = GPIO_ReadInputDataBit(_port, _pin);
